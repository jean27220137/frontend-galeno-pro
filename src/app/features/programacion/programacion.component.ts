...
@Component({
  ...
  template: `
    <div class="tw-flex tw-flex-col tw-min-h-[calc(100vh-64px)]">
      <nav class="gp-subnav" aria-label="Sub-navegación Programación">
        <div class="gp-subnav__inner">
          <span class="gp-subnav__module">
            <i class="pi pi-calendar tw-text-[#2378f0]"></i> Programación
          </span>
          @for (l of links; track l.path) {
            <a [routerLink]="l.path" routerLinkActive="gp-subnav__link--active"
               class="gp-subnav__link">
...
export class ProgramacionComponent {
  links = [
    { path: 'asignacion', label: 'Asignación Médicos', icon: 'pi pi-user-edit' },
    { path: 'horarios',   label: 'Horarios',           icon: 'pi pi-clock'     },
  ];
}
